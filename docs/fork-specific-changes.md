...
| Topic rendering | Treats Discord's topic renderer as optional. | Missing topic rendering must not make plugin startup fatal. The lookup selects Discord's two-argument `(channel, guild)` helper without depending on adjacent switch cases, so added channel types remain compatible. | Preserve the optional lookup and renderer call, distinguish the renderer from title/breadcrumb exports, and allow discovery updates as Discord changes. |
